...
FIXED_MODEL = os.getenv("GEMINI_MODEL", "gemini-flash-latest")
...
def analyze_image(image_path: str, user: UserContext, client: Optional["genai.Client"] = None) -> ImageRiskReport:
    ...
    # 이미지 검증은 tools 없이도 충분하므로 단순 호출 유지
    resp = client.models.generate_content(model=FIXED_MODEL, contents=[IMAGE_PROMPT, img])
