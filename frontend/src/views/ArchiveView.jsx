...
const RiskChart = ({ score }) => {
  ...
  return (
    <div className="relative inline-flex items-center justify-center">
      ...
      <span className="absolute text-[8px] font-black text-slate-700 dark:text-slate-200">
    ...
    <div className="max-w-6xl mx-auto px-4 py-12 space-y-12 animate-in fade-in duration-700">
      ...
      <div className="space-y-6 text-center md:text-left">
        <div className="space-y-2">
          ...
          <h1 className="text-3xl font-black leading-tight tracking-tight text-slate-900 dark:text-slate-50 sm:text-4xl md:text-5xl">
            검증 결과 아카이브
          </h1>
          <p className="max-w-2xl text-pretty text-sm font-medium leading-relaxed text-slate-500 dark:text-slate-400 sm:text-base">
            ...
            <button
              ...
              className={`cursor-pointer rounded-xl border px-5 py-2 text-xs font-black transition-all duration-200 ${
                category === cat.key
                  ...
                  : cat.key === "favorites"
                    ? "border-slate-200 bg-white text-slate-400 hover:border-rose-300 hover:bg-rose-50 hover:text-rose-600 hover:shadow-sm active:scale-[0.98] dark:border-slate-600 dark:bg-slate-800 dark:text-slate-400 dark:hover:border-rose-600 dark:hover:bg-rose-950/40 dark:hover:text-rose-300"
                    : "border-slate-200 bg-white text-slate-400 hover:border-blue-400 hover:bg-blue-50 hover:text-blue-600 hover:shadow-sm active:scale-[0.98] dark:border-slate-600 dark:bg-slate-800 dark:text-slate-400 dark:hover:border-blue-500 dark:hover:bg-slate-700 dark:hover:text-blue-300"
              }`}
            >
              {cat.key === "favorites" ? (
      ...
      <div className="space-y-3">
        <div className="rounded-3xl border border-slate-100 bg-white p-3 shadow-sm dark:border-slate-700 dark:bg-slate-900 sm:p-4">
          <form
          ...
          >
            ...
            <input
              ...
              className="w-full rounded-2xl border-none bg-slate-50 py-3 pl-11 pr-24 text-xs font-bold text-slate-800 outline-none placeholder:text-slate-400 dark:bg-slate-800/80 dark:text-slate-100 dark:placeholder:text-slate-500 sm:py-3.5 sm:pr-28 sm:text-sm"
            />
            <button
              type="submit"
        ...
        <div className="flex flex-wrap items-center justify-end gap-2 sm:justify-start">
          ...
          <div className="flex rounded-xl border border-slate-100 bg-slate-50 p-1 dark:border-slate-700 dark:bg-slate-800/80">
            <button
              type="button"
              onClick={() => {
                setSort("latest");
              ...
              className={`cursor-pointer rounded-lg px-3 py-1.5 text-[10px] font-black transition-all duration-200 sm:text-xs ${
                sort === "latest"
                  ? "bg-white text-blue-600 shadow-sm hover:bg-blue-50 dark:bg-slate-700 dark:text-blue-400 dark:hover:bg-slate-600"
                  : "text-slate-400 hover:bg-white/80 hover:text-blue-600 dark:text-slate-500 dark:hover:bg-slate-700/80 dark:hover:text-blue-400"
              }`}
            >
              최신순
            </button>
      ...
      {loading ? (
        <div className="min-h-[400px] flex flex-col items-center justify-center space-y-4">
          ...
          <p className="animate-pulse text-sm font-bold text-slate-400 dark:text-slate-500">
            최신 아카이브를 불러오는 중...
          </p>
        </div>
      ) : error ? (
        <div className="flex min-h-[400px] flex-col items-center justify-center rounded-[3rem] border border-red-100 bg-red-50 p-8 dark:border-red-900/40 dark:bg-red-950/30">
          <AlertCircle className="mb-4 h-12 w-12 text-red-500 dark:text-red-400" />
          <p className="mb-2 font-black text-red-600 dark:text-red-400">로드 실패</p>
          <button
            type="button"
        ...
        <>
            ...
            {items.length > 0 ? (
              items.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="group relative flex cursor-pointer flex-col rounded-[2rem] border border-slate-100 bg-white p-7 shadow-sm transition-all duration-200 hover:border-blue-400 hover:shadow-md dark:border-slate-700 dark:bg-slate-900 dark:hover:border-blue-500"
                  >
                    {isMy && (
                      <div className="absolute right-5 top-5 z-10 flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="flex h-9 w-9 cursor-pointer items-center justify-center rounded-xl border border-red-100 bg-white text-red-500 shadow-sm transition-all hover:border-red-200 hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-red-900/50 dark:bg-slate-800 dark:text-red-400 dark:hover:border-red-800 dark:hover:bg-red-950/40"
                    ...
                    >
                      <div className="mb-4 flex items-start pr-0">
                        <span className="rounded-lg bg-slate-100 px-2 py-1 text-[9px] font-black uppercase tracking-widest text-slate-500 dark:bg-slate-800 dark:text-slate-400">
                          {item.category_label ?? item.category_key}
                        </span>
                      </div>

                      <h3 className="mb-4 flex-grow text-lg font-bold leading-tight text-slate-800 dark:text-slate-100">
                        {item.title}
                      </h3>

                      <div className="mb-6 flex items-center gap-4 text-[10px] font-bold text-slate-400 dark:text-slate-500">
                        <span className="flex items-center gap-1">
                          <Clock size={12} /> {formatDate(item.created_at)}
                        </span>
                        <span className="flex items-center gap-1">
                          <ShieldAlert size={12} />{" "}
                          {maskAuthorDisplay(item.author_name)}
                        </span>
                      </div>

                      <div className="flex items-center justify-between border-t border-slate-50 pt-6 dark:border-slate-700/80">
                        <div className="flex items-center gap-3">
                          <RiskChart score={item.risk_score} />
                          <div>
                            <p className="text-[9px] font-black uppercase text-slate-300 dark:text-slate-600">
                              Risk Index
                            </p>
              ...
              <div className="col-span-full rounded-[2rem] border border-blue-100 bg-blue-50/50 py-20 text-center dark:border-blue-900/40 dark:bg-blue-950/30">
                <p className="mb-4 font-black text-slate-800 dark:text-slate-100">
          ...
          <div className="flex justify-center items-center gap-1 pt-8">
            <button
              ...
              className="p-2 text-slate-300 disabled:opacity-20 dark:text-slate-600"
            >
            ...
            <button
              ...
              className="mr-4 p-2 text-slate-300 disabled:opacity-20 dark:text-slate-600"
            >
              <ChevronLeft size={18} />
            </button>
            ...
            {pageButtons.map((p) => {
              ...
              return (
                <button
                  ...
                  className={
                    isActive
                      ? "h-10 w-10 rounded-xl bg-blue-600 font-black text-white shadow-lg"
                      : "h-10 w-10 rounded-xl font-black text-slate-400 hover:bg-white hover:shadow-sm dark:text-slate-500 dark:hover:bg-slate-800"
                  }
                >
                  {p}
                </button>
            ...
            <button
              ...
              className="ml-4 p-2 text-slate-300 disabled:opacity-20 dark:text-slate-600"
            >
          ...
          <div className="pt-2 text-center text-[10px] font-bold text-slate-300 dark:text-slate-600">
