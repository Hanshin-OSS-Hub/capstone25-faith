...
const SignUpView = ({ onBack, onSignupSuccess, onError }) => {
  ...
  return (
    <div className="flex items-center justify-center py-6 min-h-[80vh]">
      <div className="w-full max-w-[500px] space-y-8 rounded-[2.5rem] border border-slate-100 bg-white p-10 shadow-2xl dark:border-slate-700 dark:bg-slate-900">
        {/* Header */}
        <div className="relative">
          <button
            type="button"
            onClick={() => navigate("/login")}
            className="absolute -left-2 top-1 text-slate-400 hover:text-blue-600 dark:text-slate-500 dark:hover:text-blue-400"
          >
            <ChevronLeft size={24} />
          </button>
          <h2 className="text-center text-3xl font-black text-slate-900 dark:text-slate-50">회원가입</h2>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <div className="flex items-end gap-2">
              ...
              <button
                ...
                className="h-[52px] shrink-0 whitespace-nowrap rounded-2xl bg-slate-100 px-4 text-sm font-bold transition-all hover:bg-blue-600 hover:text-white dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-blue-600 dark:hover:text-white"
              >
              ...
              <div
                className={
                  formData.password.length >= 8
                    ...
                    : "flex items-center justify-between gap-2 text-sm font-medium text-slate-400 dark:text-slate-500"
                }
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-slate-400 dark:text-slate-500">성별</label>
          ...
          <button
            ...
            className={
              isLoading
                ? "w-full cursor-not-allowed rounded-2xl bg-slate-100 py-5 font-black text-slate-400 transition-all dark:bg-slate-800 dark:text-slate-500"
                : "w-full rounded-2xl bg-blue-600 py-5 font-black text-white transition-all hover:bg-blue-700"
            }
          >
