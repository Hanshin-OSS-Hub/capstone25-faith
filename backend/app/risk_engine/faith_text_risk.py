...
# 기본: 현재 API에서 널리 쓰이는 2.5/2.0 flash 계열 순 → 텍스트는 call_gemini에서 degrade
...
FIXED_MODEL = os.getenv("GEMINI_MODEL", DEFAULT_GEMINI_MODEL)
...
def get_model_candidates() -> list[str]:
    ...
    env_fallbacks = [m.strip() for m in fallback_env.split(",") if m.strip()]

    ordered: list[str] = []
    for model_name in [primary, *env_fallbacks, *DEFAULT_GEMINI_MODEL_CHAIN]:
...
def compute_user_coeff(user: UserContext, flags: Dict[str, Any]) -> float:
    ...
    U = 1.0
    age = user.age
    gender_raw = (user.gender or "other").strip().lower()
    # 회원 DB(M/F)·API 문자열 모두 허용 (엔진 취약군 보정은 여성 프로필만 가산)
    is_woman_profile = gender_raw in (
        "woman",
        "female",
        "f",
        "girl",
    ...
    if is_woman_profile and (sexual or deepfake or impersonation):
        U += 0.3
...
def call_gemini_with_retry(client, prompt: str, retries: int = 3):
    ...
    if last_error is not None and _is_retryable_client_error(str(last_error)):
        return None
    if last_error:
        raise last_error
...
def analyze_text(text: str, user: UserContext, client: Optional["genai.Client"] = None) -> TextRiskReport:
    ...
    if resp is None:
        agent_out = normalize_agent_output(
            {
                "text_risk": 0.0,
                "fact_risk": 0.0,
                "synthetic_risk": 0.0,
                ...
                "flags": {},
                "notes": (
                    "Gemini 호출 불가(모델 순차 시도 후에도 실패). "
                    "자동 완화 응답이며 신뢰도가 낮습니다."
                ),
            }
        )
    else:
