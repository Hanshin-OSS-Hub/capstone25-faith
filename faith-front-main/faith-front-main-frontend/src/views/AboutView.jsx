...
const verifyFlow = [
  ...
  },
  {
    ...
    step: "Step 2",
    title: "가중치 기반 앙상블",
    desc: "각 엔진의 결과를 Gemini 55%, Groq 25%, HuggingFace 20% 가중치로 통합합니다. 성능이 검증된 엔진에 더 높은 신뢰도를 부여하는 방식입니다.",
    color: "text-blue-600",
    bg: "bg-blue-50",
      ...
      <div className="border-b border-slate-100 bg-slate-50/60 px-6 py-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="text-right">
            ...
            <span className="inline-flex items-center gap-1.5 rounded-xl bg-orange-50 px-3 py-1.5 text-sm font-black text-orange-600">
        ...
        <div className="mt-4">
          ...
          <div className="mt-1 flex justify-between text-[9px] font-bold text-slate-300 tabular-nums">
            <span>0</span>
            <span>34 · MEDIUM</span>
            <span>50 · HIGH</span>
          ...
          >
            <div className="mb-3 flex items-center gap-2">
              ...
              <span className="text-[11px] font-black text-slate-500">
                Score: <span className="text-slate-800">{r.score}</span> / 100
              </span>
            </div>
            <p className="rounded-xl border border-slate-100 bg-slate-50 px-4 py-3 text-sm font-medium leading-relaxed text-slate-600">
              📌 {r.reason}
            </p>
          </div>
        ))}
...
const AboutView = () => {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 -mx-6 -mt-12 -mb-12">
            ...
            <ul className="space-y-10 text-left lg:border-l lg:border-slate-200 lg:pl-6 xl:pl-8">
              {steps.map((item) => (
                <li key={item.step} className="flex gap-4 sm:gap-5">
                  <span className="shrink-0 text-3xl font-black leading-none text-blue-600 tabular-nums" aria-hidden>
                    {item.step}
                  </span>
      ...
      >
        ...
        <div className="mb-12 md:mb-16">
          ...
          <h2 className="mb-4 text-3xl font-black leading-tight text-slate-900 md:text-4xl">
            결과에는 <span className="text-blue-600">기준</span>이 있습니다
          </h2>
          <p className="max-w-2xl text-pretty text-sm font-medium leading-relaxed text-slate-600 break-keep md:text-base">
        ...
        >
          {verifyFlow.map((item, idx) => {
            ...
            return (
              <div
                key={item.step}
                className={`relative rounded-[1.75rem] border ${item.border} ${item.bg} p-7 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-slate-200/60`}
                style={{ transitionDelay: `${idx * 80}ms` }}
              >
                {/* 연결선 화살표 (마지막 제외) */}
                {idx < verifyFlow.length - 1 && (
                  <div className="absolute -right-2.5 top-1/2 z-10 hidden -translate-y-1/2 md:flex">
                    <div className="flex h-5 w-5 items-center justify-center rounded-full bg-white shadow-sm border border-slate-100">
                      <ChevronRight size={11} className="text-slate-400" strokeWidth={2.5} />
                    </div>
                  </div>
                )}
                <div className={`mb-4 flex h-11 w-11 items-center justify-center rounded-2xl bg-white shadow-sm ${item.color}`}>
                  <Icon size={22} strokeWidth={2} />
                </div>
                <p className={`mb-1.5 text-[10px] font-black uppercase tracking-[0.18em] ${item.color}`}>
                  {item.step}
                </p>
                <h3 className="mb-3 text-base font-black text-slate-900">{item.title}</h3>
      ...
      <section
        ref={criteriaRef}
        className="relative left-1/2 mt-16 w-screen max-w-none -translate-x-1/2 border-y border-slate-100 bg-slate-50/50 py-16 md:mt-24 md:py-20"
      >
        <div className="mx-auto max-w-6xl px-6 md:px-14">
          <div className="mb-12 text-center md:mb-16">
            ...
            <h2 className="mb-4 text-3xl font-black text-slate-900 md:text-4xl">
              위험도 <span className="text-blue-600">판단 기준</span>
            </h2>
            <p className="mx-auto max-w-xl text-sm font-medium text-slate-500 break-keep">
              다음 5가지 축을 동시에 분석하고, 가중치 앙상블로 합산해 최종 점수를 산출합니다.
            </p>
          </div>
          ...
          >
            {["text_risk", "fact_risk", "synthetic_risk"].map((tag, i) => (
              <React.Fragment key={tag}>
                <span className="rounded-lg border border-blue-200 bg-white px-2.5 py-1.5 text-[11px] font-black uppercase tracking-wider text-blue-900">
                  {tag}
                </span>
                {i < 2 && (
                  <span className="px-0.5 text-2xl font-black leading-none text-blue-600" aria-hidden>
                    +
                  </span>
                )}
              </React.Fragment>
            ...
            <ArrowRight
              className="mx-1 shrink-0 text-slate-400"
              size={28}
              strokeWidth={2.75}
              aria-hidden
            />
            <span className="rounded-xl border border-blue-200 bg-blue-50 px-3 py-1.5 text-[11px] font-black uppercase tracking-wider text-blue-700">
              weighted ensemble
            </span>
            <ArrowRight
              className="mx-1 shrink-0 text-slate-400"
              ...
              aria-hidden
            />
            <span className="rounded-xl border border-slate-800 bg-slate-900 px-3 py-1.5 text-[11px] font-black uppercase tracking-wider text-white">
              risk_score 0–100
            </span>
          </div>
          ...
          >
            {riskCriteria.map((item, idx) => {
              ...
              return (
                <div
                  key={item.num}
                  className={`rounded-[1.75rem] border border-slate-100 bg-white p-7 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md hover:shadow-slate-200/50 ${
                ...
                >
                  ...
                  <h3 className="mb-2.5 text-base font-black text-slate-900">{item.title}</h3>
                  <p className="text-sm font-medium leading-relaxed text-slate-600">{item.desc}</p>
                </div>
              );
            })}
      ...
      >
        <div className="grid gap-12 lg:grid-cols-2 lg:items-center lg:gap-16">
          ...
          <div>
            ...
            <h2 className="mb-4 text-2xl font-black leading-tight text-slate-900 md:text-3xl">
              결과는 <span className="text-blue-600">점수 하나</span>로
              <br />끝나지 않습니다
            </h2>
            <p className="mb-6 text-xs font-medium leading-relaxed text-slate-600 break-keep md:text-sm">
            ...
            <ul className="space-y-2.5">
              {[
                ...
                <li key={text} className="flex items-start gap-2.5">
                  <div className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-md bg-blue-50">
                    <Icon size={11} className="text-blue-600" strokeWidth={2.5} />
                  </div>
                  <p className="text-xs font-medium leading-relaxed text-slate-700 break-keep md:text-[13px]">{text}</p>
                </li>
              ))}
            </ul>
          </div>
      ...
      <section className="mx-auto mt-16 max-w-6xl px-6 md:mt-24">
        ...
        <div className="grid gap-4 md:grid-cols-3 md:gap-6">
          {values.map(({ icon: Icon, title, lines }) => (
            ...
            >
              <Icon className="mx-auto mb-4 h-10 w-10 text-blue-600" strokeWidth={2} />
